...
function AuthJsCredentialsSignInForm() {
	...
	async function onSubmit(formData: FormType) {
		...
		const result = await signIn(email, password, formData.remember);

		if (!result.ok) {
			const message = signinErrors[result.error ?? 'default'] ?? signinErrors.default;
			setError('root', { type: 'manual', message });
			return false;
		}

		const redirectUrl = getSessionRedirectUrl() || result.redirectUrl || '/';
