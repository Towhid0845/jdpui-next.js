...
import { getVacancies } from '@/api/services/vacancies';
...
function ClientProposalsPageView() {
	...
	const { data, isLoading, error } = useQuery({
		queryKey: ['client-proposals', payload],
		queryFn: () => getVacancies(payload),
		enabled: isReady
	});
			...
			header={
				<div className="flex flex-col gap-4 p-6">
					<div className="flex flex-col gap-1">
						<Typography variant="h6">Recruiter Proposals</Typography>
						<PageBreadcrumb />
					</div>
					<Paper
						...
						<>
							<div className="flex flex-col divide-y rounded-xl border">
								{items.map((item: Record<string, unknown>, idx: number) => (
									<div
										key={(item.VacId as string | number) || idx}
...
export default ClientProposalsPageView;
