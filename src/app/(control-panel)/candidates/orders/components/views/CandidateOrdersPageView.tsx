...
import { getRecruiterOrders } from '@/api/services/sellers';
...
function CandidateOrdersPageView() {
	...
	const { data, isLoading, error } = useQuery({
		queryKey: ['candidate-orders', payload],
		queryFn: () => getRecruiterOrders(payload),
		enabled: isReady
	});
						...
						<>
							<div className="flex flex-col divide-y rounded-xl border">
								{items.map((item: Record<string, unknown>, idx: number) => (
									<div
										key={(item.OrderId as string | number) || idx}
										className="flex items-center justify-between p-4"
									>
										<div className="flex flex-col">
											<Typography className="font-medium">
												{(item.ServiceTitle as string) || '—'}
											</Typography>
											{item.Status && (
												<Typography
													variant="body2"
...
export default CandidateOrdersPageView;
